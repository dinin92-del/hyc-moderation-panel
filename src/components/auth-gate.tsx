...
export function AuthGate({
  ...
  useEffect(() => {
    return onAuthStateChanged(auth, async (u) => {
      ...
      try {
        ...
        else setState({ phase: "denied", email: u.email ?? u.displayName ?? u.uid });
      } catch {
        setState({ phase: "denied", email: u.email ?? u.displayName ?? u.uid });
      }
    });
  }, []);
  ...
  async function loginGoogle() {
    ...
    try {
      ...
      setErr("Logowanie przez Google nie powiodło się.");
    } finally {
      setBusy(false);
    }
  }
          ...
          {state.phase === "denied" ? (
            ...
            <div className="space-y-3">
              ...
              >
                ...
                {busy ? "Logowanie…" : "Zaloguj przez Google"}
              </Button>
              ...
              {!showEmail ? (
                ...
                <form onSubmit={loginEmail} className="space-y-2 pt-1 border-t">
                  <Input
                    type="email"
                    placeholder="E-mail"
                    value={email}
